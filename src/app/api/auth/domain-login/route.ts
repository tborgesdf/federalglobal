...
export async function POST(request: NextRequest) {
  try {
    ...
    */
    ...
    if (recentAttempts?.blockedUntil && recentAttempts.blockedUntil > new Date()) {
      ...
      await prisma.loginAttempt.create({
        data: {
          userId: user.id,
          ipAddress: '127.0.0.1', // IP simplificado para teste
          userAgent: request.headers.get('user-agent') || '',
        ...
        await prisma.securityAlert.create({
          data: {
            ...
            details: JSON.stringify({
              ...
              blockedUntil: blockedUntil,
              lastAttempt: {
                ip: '127.0.0.1', // IP simplificado para teste
                userAgent: request.headers.get('user-agent'),
                location: `Brasília, DF` // Localização simplificada para teste
              }
            }),
            severity: 'CRITICAL'
          }
    ...
    // SEÇÃO DE CAPTURA DE DADOS COMENTADA PARA DEBUG
    /*
    // Capturar dados completos do acesso
    const accessLogId: number | null = null
    ...
    try {
      ...
      const networkData = {
        ipAddress: '127.0.0.1', // IP simulado
        ipMapLink: '',
        country: 'Brasil',
        city: 'Brasília',
        state: 'DF',
    ...
    */
    ...
    // Criar nova sessão ativa (versão simplificada para teste)
    ...
    await prisma.activeSession.create({
      data: {
        userId: user.id,
        sessionToken,
        ipAddress: simpleNetworkData.ipAddress,
        userAgent: deviceInfo?.userAgent || request.headers.get('user-agent') || '',
        deviceInfo: JSON.stringify({
          operatingSystem: 'Windows',
          browser: 'Chrome',
          deviceType: 'Desktop',
          connectionType: simpleNetworkData.connectionType
        }),
    ...
    const loginData = {
      ...
      session: {
        ...
        accessLogId: null // Temporariamente null para teste
      },
      gps: gpsData,
      context: {
        domain: domainContext.hostname,
